...
add_wrap(int argc, char **argv)
{
    ...
    struct getargs args[] = {
        ...
        { "pw-file", 0, arg_string, NULL, "path or STDIN where password is read from", NULL },
    ...
    args[3].value = &opt.pw_file_string;
    args[4].value = &opt.password_string;
    args[5].value = &opt.salt_flag;
    args[6].value = &opt.random_flag;
    args[7].value = &opt.hex_flag;
...
copy_wrap(int argc, char **argv)
{
    ...
    int optidx = 0;
    struct getargs args[] = {
        { "match-principal", 0, arg_string, NULL, "copy only matching principals", "principal" },
        { "help", 'h', arg_flag, NULL, NULL, NULL }
    };
    int help_flag = 0;
    opt.match_principal_string = NULL;
    args[0].value = &opt.match_principal_string;
    ...
    if(help_flag)
        goto usage;
    ret = kt_copy(&opt, argc - optidx, argv + optidx);
    return ret;
usage:
    arg_printusage (args, 2, "copy", "source destination");
    return 0;
}
...
SL_cmd commands[] = {
        ...
        { "copy", copy_wrap, "copy source destination", "Copies content of one keytab into another." },
